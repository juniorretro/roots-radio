...
const TYPE_LABELS = { program: 'Programme', episode: 'Épisode' };
    ...
    <div style={{ background: 'var(--rr-bg, #fafafa)', minHeight: '100vh', paddingTop: 40, paddingBottom: 40 }}>
      <Container>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 32, flexWrap: 'wrap', gap: 12 }}>
          <div>
            <h1 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 'clamp(1.8rem,4vw,2.6rem)', fontWeight: 300, color: 'var(--rr-text, #000)', margin: 0, letterSpacing: '-0.02em' }}>Mes Favoris</h1>
            <p style={{ color: 'var(--rr-text2, #666)', fontSize: 14, marginTop: 6, marginBottom: 0 }}>{favorites.length} élément{favorites.length !== 1 ? 's' : ''} sauvegardé{favorites.length !== 1 ? 's' : ''}</p>
          </div>
          {favorites.length > 0 && (
            <button onClick={() => { if (window.confirm('Vider tous les favoris ?')) clearFavorites(); }}
              ...
              <i className="bi bi-trash me-2" />Tout effacer
            </button>
          )}
        </div>
        ...
        {favorites.length > 0 && (
          <div style={{ display: 'flex', gap: 8, marginBottom: 24, flexWrap: 'wrap' }}>
            {[{ key: 'all', label: `Tous (${favorites.length})` }, { key: 'program', label: `Programmes (${favorites.filter(f => f.type === 'program').length})` }, { key: 'episode', label: `Épisodes (${favorites.filter(f => f.type === 'episode').length})` }].map(t => (
              <button key={t.key} onClick={() => setFilter(t.key)} style={{ padding: '7px 16px', borderRadius: 20, border: `1.5px solid ${filter === t.key ? '#000' : 'rgba(0,0,0,0.12)'}`, background: filter === t.key ? '#000' : 'transparent', color: filter === t.key ? '#fff' : 'var(--rr-text, #000)', fontSize: 13, fontWeight: 500, cursor: 'pointer' }}>
                {t.label}
              </button>
            ))}
          </div>
        ...
        {favorites.length === 0 && (
          <div style={{ textAlign: 'center', padding: '80px 20px' }}>
            ...
            <h4 style={{ fontWeight: 400, color: 'var(--rr-text, #000)', marginBottom: 8 }}>Aucun favori pour l'instant</h4>
            <p style={{ color: 'var(--rr-text2, #666)', marginBottom: 24 }}>Cliquez sur le cœur sur un programme ou un épisode pour le sauvegarder ici.</p>
            <Link to="/programs" style={{ background: '#000', color: '#fff', padding: '10px 28px', borderRadius: 30, textDecoration: 'none', fontSize: 13, fontWeight: 500 }}>Voir les programmes</Link>
          </div>
        )}
                ...
                >
                  ...
                  <div style={{ padding: '14px 16px' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <span style={{ fontSize: 11, color: '#aaa' }}>Sauvegardé le {formatDate(item.savedAt)}</span>
                      {item.slug && (
        ...
        {filtered.length === 0 && favorites.length > 0 && (
          <div style={{ textAlign: 'center', padding: '60px 20px', color: 'var(--rr-text2, #666)' }}>
            Aucun {filter === 'program' ? 'programme' : 'épisode'} dans vos favoris.
          </div>
        )}
      </Container>
    </div>
