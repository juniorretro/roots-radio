...
// ════════════════════════════════════════════════════════════════════════
// AdminEpisodes.jsx — Apple Design System
// ════════════════════════════════════════════════════════════════════════
...
import { Link } from 'react-router-dom';
import { useRadio } from '../../contexts/RadioContext';
import { A, AppleLayout, AppleCard, CardHeader, AppleBtn, AppleLinkBtn, AppleBadge, AppleTable, AppleTr, AppleTd, AppleThumb, AppleInput, AppleSelect, AppleFormGroup, AppleToggle, AppleAlert, AppleEmpty, AppleSpinner, AppleModal } from './AppleAdmin.shared';

const STATUS_OPT = [
  { value:'draft',     label:'Brouillon',  variant:'gray'   },
  { value:'scheduled', label:'Programmé',  variant:'orange' },
  { value:'aired',     label:'Diffusé',    variant:'green'  },
  { value:'archived',  label:'Archivé',    variant:'gray'   },
];

const EMPTY_EP = { title:'', description:'', programId:'', season:1, episodeNumber:1, duration:3600, airDate:'', audioFile:'', image:'', featured:false, status:'draft' };
...
const AdminEpisodes = () => {
  ...
  const uploadFile = async file => {
    ...
    } catch (err) { setUploading(false); showAlert('danger',`Erreur upload: ${err.message}`); return null; }
  };

  const fetchEp = useCallback(async (p=1) => {
    setLoading(true);
    try { const d = await getEpisodes({ page:p, limit:10 }); setEpisodes(d.episodes||[]); setPages(d.totalPages||1); setPage(p); }
    catch { showAlert('danger','Erreur chargement'); }
    setLoading(false);
  }, [getEpisodes]);

  const fetchProg = useCallback(async () => {
    try { const d = await getPrograms(); setPrograms(Array.isArray(d)?d:d?.programs||[]); }
    catch {}
  }, [getPrograms]);

  useEffect(() => { fetchProg(); fetchEp(); }, [fetchProg, fetchEp]);

  const openModal = (ep = null) => {
    if (ep) { setEditing(ep); setForm({ title:ep.title||'', description:ep.description||'', programId:ep.programId?._id||ep.programId||'', season:ep.season||1, episodeNumber:ep.episodeNumber||1, duration:ep.duration||3600, airDate:ep.airDate?ep.airDate.slice(0,16):'', audioFile:ep.audioFile||'', image:ep.image||'', featured:!!ep.featured, status:ep.status||'draft' }); }
    else    { setEditing(null); setForm(EMPTY_EP); }
    setImgFile(null); setAudioFile(null); setShowModal(true);
  };

  const closeModal = () => { setShowModal(false); setEditing(null); setImgFile(null); setAudioFile(null); };
  const set = (name, val) => setForm(f => ({ ...f, [name]: val }));

  const handleSubmit = async e => {
    e.preventDefault();
    if (!form.title.trim())   { showAlert('danger','Titre requis'); return; }
    if (!form.description.trim()) { showAlert('danger','Description requise'); return; }
    if (!form.programId)      { showAlert('danger','Programme requis'); return; }
    let image = form.image, audioFile_ = form.audioFile;
    if (imgFile)   { const u = await uploadFile(imgFile);   if (u) image    = u; }
    if (audioFile) { const u = await uploadFile(audioFile); if (u) audioFile_ = u; }
    const data = { title:form.title.trim(), description:form.description.trim(), programId:form.programId, season:+form.season, episodeNumber:+form.episodeNumber, duration:+form.duration, status:form.status, featured:!!form.featured, ...(form.airDate&&{airDate:new Date(form.airDate).toISOString()}), ...(image&&{image}), ...(audioFile_&&{audioFile:audioFile_}) };
    try {
      if (editing) { await updateEpisode(editing._id, data); showAlert('success','Épisode mis à jour !'); }
      else         { await createEpisode(data);               showAlert('success','Épisode créé !'); }
      closeModal(); fetchEp(page);
  ...
  const handleDelete = async id => {
    ...
    try { await deleteEpisode(id); showAlert('success','Épisode supprimé'); fetchEp(page); }
  ...
  const statusInfo = v => STATUS_OPT.find(s => s.value===v)||STATUS_OPT[0];

  const COLS = [{ label:'Épisode', w:280 },{ label:'Programme' },{ label:'S/E' },{ label:'Durée' },{ label:'Diffusion' },{ label:'Statut' },{ label:'', w:100 }];

  return (
    <AppleLayout
      title="Épisodes"
      subtitle="Gérez les épisodes de vos programmes"
      actions={<>
        <AppleLinkBtn to="/admin" variant="ghost"><i className="bi bi-arrow-left" />Retour</AppleLinkBtn>
        <AppleBtn onClick={() => openModal()}><i className="bi bi-plus" />Nouvel Épisode</AppleBtn>
      </>}
    >
      {alert && <AppleAlert type={alert.type} onClose={() => setAlert(null)}>{alert.msg}</AppleAlert>}

      <AppleCard>
        <CardHeader title={`Épisodes (${episodes.length})`} right={<AppleBtn variant="ghost" onClick={() => fetchEp(page)}><i className="bi bi-arrow-clockwise" /></AppleBtn>} />
        {loading ? <AppleSpinner /> : episodes.length===0 ? (
          <AppleEmpty icon="collection-play" title="Aucun épisode" action={<AppleBtn onClick={() => openModal()}>Créer un épisode</AppleBtn>} />
        ) : (
              ...
              <AppleTr key={ep._id}>
                <AppleTd>
                  <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                    {ep.image && <AppleThumb src={ep.image} alt={ep.title} size={40} />}
                    <div>
                      <div style={{ fontWeight:600, fontSize:14 }}>{ep.title}</div>
                      {ep.featured && <AppleBadge variant="orange">Vedette</AppleBadge>}
                    </div>
                  </div>
                </AppleTd>
                <AppleTd><AppleBadge variant="gray">{ep.programId?.title||'—'}</AppleBadge></AppleTd>
                <AppleTd><span style={{ fontWeight:600 }}>S{ep.season}E{ep.episodeNumber}</span></AppleTd>
                <AppleTd><span style={{ fontSize:13, color:A.text2 }}>{fmtDur(ep.duration)}</span></AppleTd>
                <AppleTd><span style={{ fontSize:13, color:A.text2 }}>{fmtDate(ep.airDate)}</span></AppleTd>
                <AppleTd><AppleBadge variant={statusInfo(ep.status).variant}>{statusInfo(ep.status).label}</AppleBadge></AppleTd>
                <AppleTd>
                  <div style={{ display:'flex', gap:6 }}>
                    {ep.audioFile && <a href={ep.audioFile} target="_blank" rel="noopener noreferrer"><AppleBtn variant="success" style={{ padding:'6px 10px' }}><i className="bi bi-play-fill" style={{ fontSize:13 }} /></AppleBtn></a>}
                    <AppleBtn variant="ghost" style={{ padding:'6px 10px' }} onClick={() => openModal(ep)}><i className="bi bi-pencil" style={{ fontSize:13 }} /></AppleBtn>
                    <AppleBtn variant="danger" style={{ padding:'6px 10px' }} onClick={() => handleDelete(ep._id)}><i className="bi bi-trash" style={{ fontSize:13 }} /></AppleBtn>
                  </div>
                </AppleTd>
              </AppleTr>
            ))}
        ...
        {/* Pagination */}
        {pages > 1 && (
          <div style={{ padding:'16px 20px', borderTop:`1px solid ${A.border}`, display:'flex', justifyContent:'center', alignItems:'center', gap:12 }}>
            <AppleBtn variant="ghost" style={{ padding:'6px 14px' }} disabled={page===1} onClick={() => fetchEp(page-1)}>← Précédent</AppleBtn>
            <span style={{ fontSize:13, color:A.text2 }}>Page {page} / {pages}</span>
            <AppleBtn variant="ghost" style={{ padding:'6px 14px' }} disabled={page===pages} onClick={() => fetchEp(page+1)}>Suivant →</AppleBtn>
          </div>
        )}
      </AppleCard>

      {/* Modal */}
      <AppleModal show={showModal} onHide={closeModal} size="xl" title={editing?'Modifier l\'épisode':'Nouvel épisode'}
        footer={<>
          <AppleBtn variant="ghost" onClick={closeModal} disabled={uploading}>Annuler</AppleBtn>
      ...
      >
        <form onSubmit={handleSubmit}>
          <div style={{ display:'grid', gridTemplateColumns:'2fr 1fr', gap:16, marginBottom:16 }}>
            <AppleFormGroup label="Titre" required><AppleInput value={form.title} onChange={e=>set('title',e.target.value)} placeholder="Titre de l'épisode" required /></AppleFormGroup>
            <AppleFormGroup label="Programme" required>
              <AppleSelect value={form.programId} onChange={e=>set('programId',e.target.value)} required>
                <option value="">Choisir un programme</option>
                {programs.map(p => <option key={p._id} value={p._id}>{p.title}</option>)}
              </AppleSelect>
              {programs.length===0 && <p style={{ fontSize:12, color:A.orange, marginTop:4 }}>Aucun programme — <Link to="/admin/programs" style={{ color:A.blue }}>en créer un</Link></p>}
            </AppleFormGroup>
          </div>

          <AppleFormGroup label="Description" required>
            <AppleInput value={form.description} onChange={e=>set('description',e.target.value)} rows={3} placeholder="Description de l'épisode" required />
          </AppleFormGroup>

          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr 1fr', gap:16, marginBottom:16 }}>
            <AppleFormGroup label="Saison" required><AppleInput type="number" value={form.season} onChange={e=>set('season',+e.target.value)} min="1" required /></AppleFormGroup>
            <AppleFormGroup label="Épisode N°" required><AppleInput type="number" value={form.episodeNumber} onChange={e=>set('episodeNumber',+e.target.value)} min="1" required /></AppleFormGroup>
            <AppleFormGroup label="Durée (sec)" required hint={`≈ ${Math.floor(form.duration/60)}min`}><AppleInput type="number" value={form.duration} onChange={e=>set('duration',+e.target.value)} min="60" required /></AppleFormGroup>
            <AppleFormGroup label="Statut">
              <AppleSelect value={form.status} onChange={e=>set('status',e.target.value)}>
                {STATUS_OPT.map(s => <option key={s.value} value={s.value}>{s.label}</option>)}
              </AppleSelect>
            </AppleFormGroup>
          </div>

          <AppleFormGroup label="Date de diffusion (optionnel)">
            <AppleInput type="datetime-local" value={form.airDate} onChange={e=>set('airDate',e.target.value)} />
          </AppleFormGroup>

          {/* Audio upload */}
          <AppleFormGroup label="Fichier audio / vidéo">
            ...
            />
            {audioFile && audioFile.type.startsWith('audio/') && <audio controls src={URL.createObjectURL(audioFile)} style={{ width:'100%', marginBottom:8 }} />}
            {!audioFile && form.audioFile && <audio controls src={form.audioFile} style={{ width:'100%', marginBottom:8 }} />}
            <AppleInput value={form.audioFile} onChange={e=>set('audioFile',e.target.value)} placeholder="Ou URL audio directement" />
          </AppleFormGroup>

          {/* Image upload */}
          <AppleFormGroup label="Image">
            <input type="file" accept="image/*" onChange={e => setImgFile(e.target.files?.[0]||null)}
              style={{ width:'100%', padding:'9px 13px', background:A.bg, border:`1px solid ${A.borderStrong}`, borderRadius:10, fontSize:14, color:A.text, outline:'none', boxSizing:'border-box', marginBottom:8 }}
            />
            {imgFile && <img src={URL.createObjectURL(imgFile)} alt="preview" style={{ width:120, height:80, objectFit:'cover', borderRadius:8, marginBottom:8 }} />}
            {!imgFile && form.image && <img src={form.image} alt="current" style={{ width:120, height:80, objectFit:'cover', borderRadius:8, marginBottom:8 }} />}
            <AppleInput value={form.image} onChange={e=>set('image',e.target.value)} placeholder="Ou URL image directement" />
          </AppleFormGroup>

          <AppleToggle checked={form.featured} onChange={e=>set('featured',e.target.checked)} label="Épisode en vedette" />
        </form>
      </AppleModal>
    </AppleLayout>
  );
