...
export class StreamInfoService {
  ...
  async tryAlternativeApis() {
    ...
    try {
      const response = await fetch('http://de1.api.radio-browser.info/json/stations/byurl/' + encodeURIComponent(this.streamUrl));
...
export const useStreamInfo = (streamUrl, updateInterval = 30000) => {
  ...
  const [service] = useState(() => new StreamInfoService('http://votre-api.com/api'));
