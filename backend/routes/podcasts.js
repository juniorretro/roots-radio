  ...
  try {
    ...
    if (search) {
      ...
      query.$or = [
        ...
        { host: { $regex: safeSearch, $options: 'i' } }
      ];
    }
...
router.post('/', [
  ...
  body('audioUrl')
    .trim().notEmpty()
    .matches(/^(\/uploads\/|https:\/\/)/).withMessage('audioUrl doit être /uploads/ ou https://'),
  ...
  try {
    ...
    const { title, description, host, category, audioUrl, duration, publishDate, featured, tags, image } = req.body;
